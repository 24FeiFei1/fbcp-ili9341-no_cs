...
void InitST7735R()
{
  ...
  BEGIN_SPI_COMMUNICATION();
  {
    ...
    // Custom initialization sequence for 240x240 ST7789 without CS pin
    // Exact match to Python driver initialization
    usleep(10*1000);  // 10ms initial delay
    
    SPI_TRANSFER(0x11/*SLPOUT: Sleep Out*/);
    usleep(150*1000);  // 150ms wait after sleep out
    
    SPI_TRANSFER(0x36/*MADCTL: Memory Access Control*/, 0x00);
    SPI_TRANSFER(0x3A/*COLMOD: Pixel Format Set*/, 0x05/*16bpp*/);
    
    // Porch Control - only 2 data bytes as per Python code
    SPI_TRANSFER(0xB2/*PORCTRL: Porch Control*/, 0x0C, 0x0C, 0x00, 0x33, 0x33);
    SPI_TRANSFER(0xB7/*GCTRL: Gate Control*/, 0x35);
    SPI_TRANSFER(0xBB/*VCOMS: VCOM Setting*/, 0x1A);
    SPI_TRANSFER(0xC0/*LCMCTRL: LCM Control*/, 0x2C);
    SPI_TRANSFER(0xC2/*VDVVRHEN: VDV and VRH Command Enable*/, 0x01);
    SPI_TRANSFER(0xC3/*VRHS: VRH Set*/, 0x0B);
    SPI_TRANSFER(0xC4/*VDVSET: VDV Set*/, 0x20);
    SPI_TRANSFER(0xC6/*FRCTR2: Frame Rate Control in Normal Mode*/, 0x0F);
    SPI_TRANSFER(0xD0/*PWCTRL1: Power Control 1*/, 0xA4, 0xA1);
    
    // INVERT AYARI
    SPI_TRANSFER(0x21/*INVON: Display Inversion On*/);
    
    //SPI_TRANSFER(0x20/*INVON: Display Inversion Off*/);

    // Positive Voltage Gamma Control - 14 parameters
    //SPI_TRANSFER(0xE0/*PVGAMCTRL*/, 0x00, 0x19, 0x1E, 0x0A, 0x09, 0x15, 0x3D, 0x44, 0x51, 0x12, 0x03, 0x00, 0x3F, 0x3F);
    
    // Negative Voltage Gamma Control - 14 parameters
    //SPI_TRANSFER(0xE1/*NVGAMCTRL*/, 0x00, 0x18, 0x1E, 0x0A, 0x09, 0x25, 0x3F, 0x43, 0x52, 0x33, 0x03, 0x00, 0x3F, 0x3F);
    
    // Daha yumuşak gamma
    SPI_TRANSFER(0xE0, 0x00,0x15,0x1B,0x06,0x05,0x10,0x30,0x38,0x40,0x0F,0x03,0x00,0x3A,0x3F);
    SPI_TRANSFER(0xE1, 0x00,0x14,0x1B,0x06,0x05,0x12,0x36,0x38,0x42,0x30,0x03,0x00,0x3A,0x3F);



    SPI_TRANSFER(0x29/*DISPON: Display ON*/);
    usleep(100*1000);  // 100ms wait after display on
#else
